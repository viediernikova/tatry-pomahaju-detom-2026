...
function initDiaryFeed(diary) {
  ...
  diary.forEach((post, index) => {
    ...
    if (post.photos && post.photos.length > 0) {
      const countClass = `photos-count-${Math.min(5, post.photos.length)}`;
      ...
      <div class="diary-card-body" id="body-${postId}">
        ...
        <div class="diary-actions-row">
          <div class="interaction-group">
            ...
            ${post.facebookUrl ? `
              <a href="${post.facebookUrl}" target="_blank" rel="noopener noreferrer" class="action-btn" aria-label="Zobraziť na Facebooku">
